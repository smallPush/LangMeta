...
def test_logs_ui_inaccessible_with_query_param(monkeypatch):
    """Verify that /logs/ui is not accessible with an api_key query parameter."""
    from app.config import settings
    from app.main import api_logger
    api_logger.clear_logs()
    monkeypatch.setattr(settings, "api_key", "secure_key")
...
def test_logs_ui_accessible_with_header(monkeypatch):
    """Verify that /logs/ui is accessible with the correct X-API-Key header."""
    from app.config import settings
    from app.main import api_logger
    api_logger.clear_logs()
    monkeypatch.setattr(settings, "api_key", "secure_key")
